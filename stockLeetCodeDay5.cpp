/*
Best Time to Buy and Sell Stock II - LeetCode Challenge Day 5
...
Then buy on day 4 (price = 3) and sell on day 5 (price = 6), profit = 6 - 3 = 3. Total 
profit = 4 + 3 = 7. 
...
class Solution {
	...
	dp[i][j] - max possible amount of money to get to moment i with j items in hand.

	O(N*K) solution for problem:

		dp[i][j] -> dp[i+1][j] 
		dp[i][j] -> dp[i+1][j-1] 
			==> represents getting rid of an item of stock => dp[i+1][j-1] = max(..., dp[i][j] + price[i])
		dp[i][j] -> dp[i-1][j+1]		
	*/ 
public: 
